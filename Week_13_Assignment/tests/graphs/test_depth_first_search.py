...
def run_dfs(graph, start):
    ...
    DepthFirstSearch(graph, start, tracking_draw, _noop_wait, _noop_events)
...
class TestDepthFirstSearch(unittest.TestCase):
    def test_visits_all_connected_nodes(self):
        order = run_dfs(SIMPLE, 'A')
        self.assertEqual(set(order), set(SIMPLE.keys()))

    def test_start_node_visited_first(self):
        for start in SIMPLE:
            order = run_dfs(SIMPLE, start)
            self.assertEqual(order[0], start)

    def test_dfs_order_goes_deep(self):
        order = run_dfs(SIMPLE, 'A')
        self.assertEqual(order[0], 'A')
        second = order[1]
        self.assertIn(second, {'B', 'C'})
        self.assertIn(order[2], set(SIMPLE[second]) - {'A'})

    def test_linear_graph_order(self):
        order = run_dfs(LINEAR, 'A')
        self.assertEqual(order, ['A', 'B', 'C', 'D'])

    def test_single_node(self):
        graph = {'A': []}
        order = run_dfs(graph, 'A')
    ...
    def test_abort_mid_search(self):
        ...
        result = DepthFirstSearch(SIMPLE, 'A', _noop_draw, _noop_wait, abort_after_2)
        self.assertFalse(result)

    def test_returns_true_on_completion(self):
        result = DepthFirstSearch(SIMPLE, 'A', _noop_draw, _noop_wait, _noop_events)
    ...
    def test_benchmark(self):
        ...
        start = time.time()
        DepthFirstSearch(graph, (0, 0), _noop_draw, _noop_wait, _noop_events)
        elapsed = time.time() - start
        print(f"The depth first search benchmark of traversing a {size*size:,} node grid took {elapsed:.4f} seconds.")
